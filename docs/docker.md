...
- `apps/api/.dockerignore` - Backend build context exclusions
- `apps/dashboard/Dockerfile` - Production frontend image (Nginx)
- `apps/dashboard/Dockerfile.dev` - Development frontend image
- `apps/dashboard/.dockerignore` - Frontend build context exclusions
- `apps/dashboard/nginx.conf` - Nginx configuration for SPA routing
